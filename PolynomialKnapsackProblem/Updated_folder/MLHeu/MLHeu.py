...
if __name__ == '__main__':
	...
	it_file=2
	for percentage_to_fix in np.arange(0.2,0.25,0.05):
		it_file+=1
		print(f"With percentage {percentage_to_fix}")
		count=0
		list_of_files = os.listdir(CONFIG_PATH)
		RESULTS_PATH=RESULTS_FOLDER+f"resultsMLHeu_{it_file}.txt"
		for name_file in list_of_files:
			count+=1
			if count>=783:
				print("\tDoing file {} ({} of {})".format(name_file,count,len(list_of_files)))
				...
				while len(list_ymlP_0)>percentage_to_fix*count_0:
					ele=list_ymlP_0.pop()
					y_ml[ele[1]]=0

				while len(list_ymlP_1)>percentage_to_fix*count_1:
